...
| 기준 모델 | **EfficientViT-B1 224×224** (config: `efficientvit_b1.yaml`, pretrained: `efficientvit_b1_r224.pt`) |
| 학습 방식 | **Fine-tuning** — pretrained B1 weight 로드 후 Soft Pruning 적용 재학습 |
| 방법론 | Soft Pruning (학습 중 0 마스킹) → Reducing (학습 후 dense 변환) |
| Prunable 그룹 | `MBConv.mid_channels`, `FusedMBConv.mid_channels`, **`input_stem` chain** (stage0 채널), **`ClsHead` G_HEAD0/G_HEAD1** (width_list 내부 hidden) |
...
| Opt-in 플래그 | `--target_compression <0~1>` (0 이면 pruning 비활성), `--init_from <ckpt>` (pretrained weight) |
| Pretrained weight | `assets/checkpoints/efficientvit_cls/efficientvit_b1_r224.pt` |
...
9. [실행 명령어 가이드](#9-실행-명령어-가이드) — Base Model eval(§9.0), Fine-tune, measure_memory 포함
...
| **`ClsHead` G_HEAD0** ([§5.2.2](#522-clshead-마스킹-_prune_head0--_prune_head1)) | `head.op_list[0]` Conv → `op_list[2]` Linear 입력 | 1×1 Conv(in_ch→w0) 출력 필터 + BN + FC 입력 컬럼 연동. AdaptiveAvgPool2d 가 채널 순서를 보존하므로 동일 idx 적용 가능. |
| **`ClsHead` G_HEAD1** ([§5.2.2](#522-clshead-마스킹-_prune_head0--_prune_head1)) | `head.op_list[2]` Linear 출력 → `op_list[3]` Linear 입력 | FC(w0→w1) 출력 행 + LayerNorm + 최종 FC 입력 컬럼 연동. G_HEAD1 sparsity 상한 = 0.40 (내부 고정, under-complete 방지). |
...
| `ClsHead` (`Conv → Linear → Linear`) | ~~1차 구현에서 제외~~ → **§5.2.2 에서 구현 완료.** `head_sparsity_scale` 로 backbone 대비 별도 sparsity 제어. G_HEAD1 은 1.6× near-complete 라 내부 상한 0.40 적용. |
...
### 5.2.2 ClsHead 마스킹 (`_prune_head0` / `_prune_head1`)

ClsHead 구조 (B1 기준):

```
op_list[0]  ConvLayer(256 → 1536, k=1, BN2d, HSwish)
op_list[1]  AdaptiveAvgPool2d(1)          ← 채널 순서 보존
op_list[2]  LinearLayer(1536 → 1600, bias=False, LayerNorm, HSwish)
op_list[3]  LinearLayer(1600 → 1000, bias=True)  ← n_classes 고정
```

**G_HEAD0** — `op_list[0]` 출력 필터 기준 L2 norm, `head_sp = backbone_sp × scale`:

```python
def _prune_head0(head, sparsity):
    weight = head.op_list[0].conv.weight   # (1536, 256, 1, 1)
    idx = _topk_smallest_l2_idx(weight, _calc_n_prune(1536, sparsity))
    head.op_list[0].conv.weight.data[idx] = 0.0
    _zero_bn_(head.op_list[0].norm, idx)            # BN2d
    head.op_list[2].linear.weight.data[:, idx] = 0.0  # 입력 컬럼
```

`AdaptiveAvgPool2d` 는 `(B, C, H, W) → (B, C, 1, 1)` 로 공간 축만 풀링. 채널 i 는 독립적으로
처리되어 채널 순서가 유지되므로 `op_list[0]` 의 출력 인덱스 = `op_list[2]` 의 입력 인덱스가 정확히 일치.

**G_HEAD1** — `op_list[2]` 출력 행 기준 L2 norm, `effective_sp = min(head_sp, 0.40)`:

```python
def _prune_head1(head, sparsity):
    effective_sp = min(sparsity, _HEAD1_MAX_SPARSITY)   # 0.40 상한
    weight = head.op_list[2].linear.weight   # (1600, 1536)
    idx = _topk_smallest_l2_idx(weight, _calc_n_prune(1600, effective_sp))
    head.op_list[2].linear.weight.data[idx] = 0.0
    _zero_bn_(head.op_list[2].norm, idx)                # LayerNorm (weight/bias만)
    head.op_list[3].linear.weight.data[:, idx] = 0.0   # 입력 컬럼
    # op_list[3].bias (1000) 는 출력 소속 → 건드리지 않음
```

G_HEAD1 에 0.40 상한을 두는 이유: `1600 → 1000` 은 1.6× 확장에 불과해 1600을 800 미만으로
줄이면 `800 → 1000` 의 under-complete 매핑이 되어 표현력이 급감한다.

**`EfficientViTPruner.apply()` 내 호출 순서**:
...
# head (head_sparsity_scale > 0 일 때만)
head_sp = self.sparsity * self.head_sparsity_scale
_prune_head0(model.head, head_sp)
_prune_head1(model.head, head_sp)
```

**`head_sparsity_scale` 인자 (기본값 0.5)**:

| 인자 | 설명 |
|------|------|
| `head_sparsity_scale=0.5` | head sparsity = backbone sparsity × 0.5. backbone 절반 강도. |
| `head_sparsity_scale=0.0` | head pruning 완전 비활성 (기존 동작과 동일). |
| `head_sparsity_scale=1.0` | backbone 과 동일 sparsity (G_HEAD1 은 0.40 cap 적용). |

이진탐색 시 head 제거량도 `_estimate_removed_head0/1` 을 통해 합산되어 더 낮은 backbone sparsity 로 동일 target_compression 을 달성한다 (§8.5 참조).
...
### 6.2.2 ClsHead Reducing (`_reduce_head0` / `_reduce_head1`)

**반드시 G_HEAD0 → G_HEAD1 순서**로 실행해야 한다. G_HEAD0 이 `op_list[2].linear.in_features` 를 변경하고, G_HEAD1 이 그 결과를 읽기 때문이다.

**G_HEAD0**:

```python
survived = _survived_idx(head.op_list[0].conv.weight)   # (n_new,)
# op_list[0]: ConvLayer 출력 필터 + BN 교체 (기존 _reduce_convlayer_out 재사용)
_reduce_convlayer_out(conv_layer, survived, in_channels=in_ch, groups=1)
# op_list[2].linear: in_features 축소 (out_features, LayerNorm 은 그대로)
new_lin = nn.Linear(n_new, old_lin.out_features, bias=...)
new_lin.weight.data.copy_(old_lin.weight.data[:, survived])
head.op_list[2].linear = new_lin
```

**G_HEAD1**:

```python
survived = _survived_idx(head.op_list[2].linear.weight)  # G_HEAD0 후 갱신된 linear 기준
# op_list[2].linear: out_features 축소
new_lin2 = nn.Linear(lin2.in_features, n_new, ...)
new_lin2.weight.data.copy_(lin2.weight.data[survived])
# op_list[2].norm: LayerNorm normalized_shape 축소
new_ln = nn.LayerNorm(n_new, eps=...)
new_ln.weight.data.copy_(old_ln.weight.data[survived])
new_ln.bias.data.copy_(old_ln.bias.data[survived])
# op_list[3].linear: in_features 축소 (out_features=n_classes, bias 는 그대로)
new_lin3 = nn.Linear(n_new, old_lin3.out_features, ...)
new_lin3.weight.data.copy_(old_lin3.weight.data[:, survived])
```

`reduce_efficientvit_cls_model` 내 호출:

```python
_reduce_input_stem(model)
_reduce_head0(model)   # 1st: op_list[2] in_features 변경
_reduce_head1(model)   # 2nd: op_list[2] out_features + op_list[3] in_features 변경
```
...
## 8.5 실측 기준 수치 — EfficientViT-B1 Pre-training Baseline

> weight 없이 `measure_memory.py --per-stage` 로 측정한 원본 아키텍처 파라미터 분포.
> 학습 전 baseline 으로 기록하며, 이후 reducing 결과와의 비교 기준이 된다.
...
```
=== [model] per-stage parameter memory ===
name                   numel          MB        %
--------------------------------------------------
...
| 항목 | 수치 | 비고 |
|------|------|------|
| Total params | 9,115,467 (36.5 MB) | 알려진 B1 공식 파라미터 수와 일치 |
| G_FUSEDMB = 0 | 예상대로 | B-series 에는 FusedMBConv 없음 |
| G_OTHER = 0 | 예상대로 | 모든 파라미터가 그룹에 분류됨 |
| G_HEAD = 48.94% | ⚠️ 보고서 §12.1 추정 "~30%" 와 차이 | 실제 head 가 backbone 보다 큼 |
| G_STEM = 1 모듈 | §9.4 예시 스키마의 `#mod=2` 와 다름 | measure_memory.py 가 stem 전체를 1 unit 으로 집계. 수치(1,027)는 per-stage 와 동일하므로 정합성 OK |

**Prunable / Non-prunable 비율 (실측)**
...
**target_compression 별 backbone per-group sparsity 추정**

Head-only (`head_sparsity_scale=0.0`, 기존 동작):
...
| 0.20 | ≈ 0.63 | |
| 0.25 | ≈ 0.79 | |
| **0.30** | **≈ 0.93–0.94** | ⚠️ 극단적 — 표현력 급감 |

Head 포함 (`head_sparsity_scale=0.5`, **권장**):

| target_compression | 예상 backbone sparsity | head sparsity (×0.5) | 비고 |
|-------------------|----------------------|---------------------|------|
| 0.15 | ≈ 0.22–0.27 | ≈ 0.11–0.14 | backbone 부담 절반 이하 |
| 0.20 | ≈ 0.30–0.36 | ≈ 0.15–0.18 | |
| 0.25 | ≈ 0.39–0.46 | ≈ 0.20–0.23 | |
| **0.30** | **≈ 0.47–0.55** | **≈ 0.24–0.28** | head 포함으로 backbone 0.94→0.51 수준으로 완화 |

> Head(48.94%) 를 포함하면 prunable 영역이 32% → 81% 로 확대되어
> 동일 target 에서 backbone sparsity 가 크게 낮아진다.
> **실용 권장**: `--target_compression 0.15~0.30 --pruning_head_sparsity_scale 0.5`

> 서버에서 정확한 값 확인: 학습 시작 시 출력되는
> `[EfficientViTPruner] target=30.0% backbone_sparsity=X.XXXX ... head_sparsity=X.XXXX` 로그.

---

## 9. 실행 명령어 가이드

> **모든 학습은 Fine-tuning 방식** — pretrained B1 weight 로드 후 재학습.
>
> | 항목 | 값 |
> |------|-----|
> | 기준 모델 | `efficientvit-b1` (config: [`efficientvit_b1.yaml`](applications/efficientvit_cls/configs/imagenet/efficientvit_b1.yaml)) |
> | Pretrained weight | `assets/checkpoints/efficientvit_cls/efficientvit_b1_r224.pt` |
> | 데이터 | `/workspace/etri_iitp/JS/efficientvit2026/data/imagenet` |

> **GPU 설정 주의사항**
> - `CUDA_VISIBLE_DEVICES` 에 나열한 GPU 수와 `--nproc_per_node` 를 **반드시 일치**시킨다.
> - 예: GPU 4번 1장 → `CUDA_VISIBLE_DEVICES=4 ... --nproc_per_node=1`
> - 예: GPU 4,5,6,7 4장 → `CUDA_VISIBLE_DEVICES=4,5,6,7 ... --nproc_per_node=4`

> **data_dir override**: YAML 의 `data_provider.data_dir` 는 CLI 인자로 덮어씀 (공백으로 분리, `=` 사용 금지).

### 9.0 Base Model Validation (Pretrained B1, 단발 평가)

Fine-tune 없이 pretrained B1 weight 로 ImageNet val 정확도를 측정한다. 이 수치가 모든 pruning 실험의 **비교 기준(baseline)**이 된다.

스크립트: [`applications/efficientvit_cls/eval_efficientvit_cls_model.py`](applications/efficientvit_cls/eval_efficientvit_cls_model.py)
...
CUDA_VISIBLE_DEVICES=7 python applications/efficientvit_cls/eval_efficientvit_cls_model.py \
  ...
  --wandb_run_name b1_base_pretrained_eval
```

| 인자 | 설명 |
|------|------|
| `--model efficientvit-b1` | B1 아키텍처 선택 |
| `--weight_url` | pretrained checkpoint 경로 (로컬 파일 경로도 가능) |
| `--path` | ImageNet val 디렉터리 (`/val` 하위에 클래스 폴더 구조) |
| `--gpu 7` | GPU 7번 단일 사용 (`CUDA_VISIBLE_DEVICES` 와 일치) |
| `--batch_size 128` | 단일 GPU 기준 배치 크기 |
| `--wandb` | wandb 결과 저장 활성화 |
| `--wandb_run_name` | wandb run 이름 (동일 project 내 baseline 으로 식별) |

> **참고**: 이 스크립트는 `torchrun` 불필요 — 단순 `python` 으로 실행. wandb 에 `val/top1`, `val/top5` 가 기록된다.

---

### 9.1 Fine-tune Baseline (pruning 비활성)

pretrained B1 weight 로드 후 pruning 없이 fine-tune. 압축 전 정확도 기준선.
...
CUDA_VISIBLE_DEVICES=4 torchrun --nproc_per_node=1 --master_port=12345 \
  applications/efficientvit_cls/train_efficientvit_cls_model.py \
    ...
    --path /workspace/etri_iitp/JS/efficientvit2026/output/b1_finetune_baseline \
    --init_from assets/checkpoints/efficientvit_cls/efficientvit_b1_r224.pt \
    --amp bf16 \
    --wandb \
    --wandb_project efficientvit-pruning \
    --wandb_run_name b1_finetune_baseline \
...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=12345 \
  applications/efficientvit_cls/train_efficientvit_cls_model.py \
    ...
    --wandb \
    --wandb_project efficientvit-pruning \
    --wandb_run_name b1_finetune_baseline_4gpu \
    --data_provider.data_dir /workspace/etri_iitp/JS/efficientvit2026/data/imagenet
```

### 9.2 Soft Pruning Fine-tune

pretrained B1 weight 로드 후 pruning 적용 fine-tune. **target_compression 0.15 권장** (per-group sparsity ≈ 47%).
...
CUDA_VISIBLE_DEVICES=3,4 torchrun --nproc_per_node=2 --master_port=12345 \
  applications/efficientvit_cls/train_efficientvit_cls_model.py \
    ...
    --wandb \
    --wandb_project efficientvit-pruning \
    --wandb_run_name b1_prune15_4gpu \
    --data_provider.data_dir /workspace/etri_iitp/JS/efficientvit2026/data/imagenet
```
...
> **주의**: target ≥ 0.30 은 prunable 영역의 94% 이상을 마스킹하는 극단적 수준으로
> fine-tune 만으로 정확도 회복이 어려움. 사용 시 epoch 수를 충분히 확보할 것.

- 학습 시작 직후 `[EfficientViTPruner] per-group sparsity=X.XXXX` 로그로 실제 sparsity 확인.
- 처음 1~2 epoch 후 wandb `pruning/actual_sparsity` 지표로 실제 zero 비율 검증.
...
### 9.2-C. Head Pruning 포함 (권장 — `head_sparsity_scale=0.5`)

기존 명령어에 `--pruning_head_sparsity_scale 0.5` 만 추가하면 된다.
`head_sparsity_scale` 기본값이 이미 0.5 이므로, `--target_compression` 만 쓰면 자동 활성.
비활성하려면 `--pruning_head_sparsity_scale 0.0` 을 명시한다.

GPU 1장 (GPU 7번), target=15%, head 포함:

```bash
CUDA_VISIBLE_DEVICES=7 torchrun --nproc_per_node=1 --master_port=12345 \
  applications/efficientvit_cls/train_efficientvit_cls_model.py \
    applications/efficientvit_cls/configs/imagenet/efficientvit_b1.yaml \
    --path /workspace/etri_iitp/JS/efficientvit2026/output/b1_prune15_head \
    --init_from assets/checkpoints/efficientvit_cls/efficientvit_b1_r224.pt \
    --amp bf16 \
    --target_compression 0.15 \
    ...
    --wandb \
    --wandb_project efficientvit-pruning \
    --wandb_run_name b1_prune15_head05 \
    --data_provider.data_dir /workspace/etri_iitp/JS/efficientvit2026/data/imagenet
```

GPU 1장 (GPU 7번), target=30%, head 포함:

```bash
CUDA_VISIBLE_DEVICES=7 torchrun --nproc_per_node=1 --master_port=12345 \
  applications/efficientvit_cls/train_efficientvit_cls_model.py \
    applications/efficientvit_cls/configs/imagenet/efficientvit_b1.yaml \
    --path /workspace/etri_iitp/JS/efficientvit2026/output/b1_prune30_head \
    --init_from assets/checkpoints/efficientvit_cls/efficientvit_b1_r224.pt \
    --amp bf16 \
    --target_compression 0.30 \
    --pruning_head_sparsity_scale 0.5 \
...
> 학습 시작 로그로 확인:
> `[EfficientViTPruner] target=15.0% backbone_sparsity=0.XXXX ... head_sparsity=0.XXXX (scale=0.5)`
>
> 기존 15%/30% 실험 (head 없음) 과 비교하려면 wandb 에서 같은 project 아래
> `b1_prune15` vs `b1_prune15_head05` 를 나란히 보면 된다.

**head pruning 비활성 (기존 동작 유지)**:
...
# 2) fine-tune baseline weight 로드 후
python applications/efficientvit_cls/measure_memory.py \
    --model efficientvit-b1 \
    --checkpoint /workspace/etri_iitp/JS/efficientvit2026/output/b1_finetune_baseline/checkpoint/model_best.pt

# 3) 원본 vs Reduced 비교 (reducing 완료 후)
python applications/efficientvit_cls/measure_memory.py \
    --model efficientvit-b1 \
    --reduced /workspace/etri_iitp/JS/efficientvit2026/output/b1_prune15/reduced_b1_15pct.pt \
    --per-stage
...
### 12.1 `ClsHead` (Conv → Linear → Linear) — ✅ 구현 완료 (2026-05-03)

§5.2.2, §6.2.2 참조. `head_sparsity_scale` 인자로 backbone 대비 head 압축 강도 조절.
reducing 은 `reduce_efficientvit_cls_model` 에서 자동으로 G_HEAD0 → G_HEAD1 순서로 처리.
...
| Step 3 | ClsHead G_HEAD0 마스킹 (`_prune_head0`) | ✅ §5.2.2 |
| Step 3 | ClsHead G_HEAD1 마스킹 (`_prune_head1`, 0.40 상한) | ✅ §5.2.2 |
| Step 6 | ClsHead G_HEAD0 reducing (`_reduce_head0`) | ✅ §6.2.2 |
| Step 6 | ClsHead G_HEAD1 reducing + LayerNorm 교체 (`_reduce_head1`) | ✅ §6.2.2 |
| Step 6 | G_HEAD0 → G_HEAD1 순서 보장 | ✅ `reduce_efficientvit_cls_model` |

*작성 일자: 2026-04-27. 최종 수정: 2026-05-02. 기준 코드: EfficientViT classification 파이프라인 (master, commit 53e8795 시점).*
...
**배경**: 기존 §9.1 의 Fine-tune Baseline 명령어는 학습을 요구하지만, pretrained B1 weight 의 원본
정확도를 단발 eval 로 확인하려면 별도 평가 스크립트가 필요했다. 초기 안내 시 `EfficientViT- Example/`
(M-series M4) 경로의 eval 스크립트를 사용하는 오류가 있어 B1 (`applications/efficientvit_cls/`) 경로로 수정.

**추가**
- `applications/efficientvit_cls/eval_efficientvit_cls_model.py`:
  - `--wandb`, `--wandb_project`, `--wandb_run_name` 인자 추가.
  - 평가 완료 후 `wandb.log({"val/top1": ..., "val/top5": ...})` 기록.
- 보고서 §9.0 신규: Base Model Validation 명령어 (GPU 7번, B1 pretrained, wandb 포함).

**수정**
- 보고서 §9 TOC 설명에 `Base Model eval(§9.0)` 추가.
- `작성 일자` 각주를 최종 수정 2026-05-02 로 갱신.

---

### 2026-05-03 (rev 5) — ClsHead Pruning 구현

**배경**: 기존 backbone-only pruning 에서 target=0.30 시 backbone per-group sparsity ≈ 0.94 로
MBConv hidden 의 94% 가 0 이 되어 정확도 급락. B1 head 가 전체 파라미터의 48.94% 를 차지하므로
head 를 pruning 영역에 포함하면 동일 target 에서 backbone sparsity 를 0.51 수준으로 완화 가능.

**추가**
- `efficientvit/clscore/pruning/efficientvit_pruning.py`:
  - `_HEAD1_MAX_SPARSITY = 0.40` 상수: G_HEAD1 (`width_list[1]→n_classes`) 의 under-complete 방지.
  - `_prune_head0(head, sparsity)`: op_list[0] Conv 출력 필터 + BN → op_list[2] Linear 입력 컬럼 동기 마스킹.
  - `_prune_head1(head, sparsity)`: op_list[2] Linear 출력 행 + LayerNorm → op_list[3] Linear 입력 컬럼 동기 마스킹. effective_sp = min(sp, 0.40).
  - `_estimate_removed_head0/1(head, sparsity)`: 이진탐색 추정 함수.
  - `_has_prunable_head(model)`: 안전 가드 (op_list 구조 검사).
  - `_estimate_total_removed`: `head_sparsity_scale` 인자 추가, head 추정량 합산.
  - `_find_sparsity_by_bisection`: `head_sparsity_scale` 인자 전달.
  - `EfficientViTPruner.__init__`: `head_sparsity_scale=0.5` 인자 추가, 로그에 head_sparsity 출력.
  - `EfficientViTPruner.apply()`: head pruning 호출 추가.
  - `EfficientViTPruner.log_sparsity()`: G_HEAD0/G_HEAD1 zero 비율 집계 추가.
- `efficientvit/clscore/pruning/efficientvit_reducing.py`:
  - `_reduce_head0(model)`: G_HEAD0 reducing (ConvLayer + BN + Linear in_features).
  - `_reduce_head1(model)`: G_HEAD1 reducing (Linear out_features + LayerNorm + Linear in_features). LayerNorm `normalized_shape` 교체 포함.
  - `reduce_efficientvit_cls_model`: `_reduce_head0` → `_reduce_head1` 호출 추가 (순서 필수).
- `applications/efficientvit_cls/train_efficientvit_cls_model.py`:
  - `--pruning_head_sparsity_scale` (float, default=0.5) argparse 인자 추가.
  - `EfficientViTPruner` 생성 시 `head_sparsity_scale` 전달.

**수정**
- 보고서 §0 Prunable 그룹에 G_HEAD0/G_HEAD1 추가.
- 보고서 §4.1 Prunable 표에 G_HEAD0/G_HEAD1 행 추가.
- 보고서 §4.2 Non-Prunable 표에서 ClsHead 항목을 "구현 완료" 로 수정.
- 보고서 §5.2.2 신규: ClsHead 마스킹 상세 (채널 흐름, 코드, scale 표).
- 보고서 §6.2.2 신규: ClsHead Reducing 상세 (G_HEAD0→G_HEAD1 순서 주의사항).
- 보고서 §8.5 sparsity 추정 표를 backbone-only / head 포함 두 버전으로 갱신.
- 보고서 §9.2-C 신규: GPU 7번 단일 GPU 기준 head 포함 실행 명령어 (target 15%/30%).
- 보고서 §12.1 상태를 "✅ 구현 완료" 로 갱신.
...
1. 왜 30% 타겟인데 특정 레이어는 훨씬 많이 줄었나?
핵심은 "전체 파라미터 30% 감소"와 "각 레이어별 감소율"이 다르다는 것이다.

에러 메시지에서 보면:


input_stem.op_list.0.conv.weight: [4, 3, 3, 3]  ←  원래 [16, 3, 3, 3]
이 레이어만 보면 75% 감소다.

왜 이렇게 되냐면:


전체 파라미터 중 Prunable 비율
────────────────────────────────────────────
전체 파라미터          9.1M  (100%)
Prunable 그룹          ~3.6M ( ~40%)  ← MBConv/FusedMBConv hidden + stem
Non-prunable 그룹      ~5.5M ( ~60%)  ← attention, head 등
Non-prunable은 건드리지 않으니, 전체 30% 감소를 달성하려면:


Prunable 파라미터에서 제거해야 할 양 = 9.1M × 0.30 = 2.73M
필요한 Prunable 내 sparsity = 2.73M / 3.6M ≈ 0.75 (75%)
→ 개별 레이어 채널의 75%가 0으로 마스킹 → reducing 후 75% 채널 제거

→ 전체 파라미터 기준으로는 30% 감소

특히 input_stem(16채널)처럼 작은 레이어는 75% sparsity면 16 × 0.25 = 4채널만 살아남는 것이다.

2. reduced_b1_30pct.pt vs reduced_b1_30pct_full.pt 차이
reduced_b1_30pct.pt	reduced_b1_30pct_full.pt
저장 방식	torch.save({"state_dict": ..., "compression_rate": 30.0, ...})	torch.save(model)
포함 내용	가중치 값만 (dict)	아키텍처 + 가중치 (nn.Module 객체 전체)
로드 방법	동일 아키텍처 모델을 먼저 만들어야 함	torch.load() 한 번으로 바로 사용 가능
크기	더 작음	약간 더 큼 (구조 정보 포함)
문제점	표준 B1 아키텍처와 shape가 달라서 직접 로드 불가	weights_only=False 필요 (pickle 기반)
쉽게 말하면:
