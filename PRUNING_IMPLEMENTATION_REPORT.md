...
**target_compression 별 per-group sparsity 추정** (코드 분석 기반, 서버 실측 전 참고용)

| target_compression | target_remove (params) | prunable 대비 | 예상 per-group sparsity |
|-------------------|----------------------|--------------|------------------------|
| 0.10 | ~912,000 | ~31% | ≈ 0.31 |
| 0.15 | ~1,367,000 | ~47% | ≈ 0.47 |
| 0.20 | ~1,823,000 | ~62% | ≈ 0.63 |
| 0.25 | ~2,279,000 | ~78% | ≈ 0.79 |
| **0.30** | **~2,735,000** | **~94%** | **≈ 0.93–0.94** |

> `target_compression=0.30` 은 per-group sparsity ≈ 0.94 로 각 MBConv hidden dim 의
> 94% 를 0 으로 마스킹하는 것과 동일 → 모델 용량이 극단적으로 감소.
> **실용적인 target 은 0.15~0.20 권장** (per-group sparsity 47~63%, 모델 표현력 유지).

> 서버에서 정확한 sparsity 를 확인하려면 학습 시작 시 출력되는
> `[EfficientViTPruner] target=30.0% per-group sparsity=X.XXXX ...` 로그를 확인.
...
`B1` 기준 head 가 전체 파라미터의 **48.94%** (실측; §8.5) 를 차지해 backbone 전체(51%)와 맞먹는 수준이므로, 의미 있는 추가 압축을 위해선 우선순위 후보.
