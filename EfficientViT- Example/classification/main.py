...
def get_args_parser():
    ...
    parser.add_argument('--data-set', default='IMNET',
                        choices=['CIFAR', 'IMNET', 'IMNET10', 'INAT', 'INAT19'],
                        type=str, help='Image Net dataset path')
    parser.add_argument('--subset-classes', default='', type=str,
                        help='IMNET10 전용: 쉼표 구분 클래스 폴더명 (예: "00000,00001,..."). '
                             '비워두면 알파벳 순 첫 10개 사용')
    ...
    parser.add_argument('--wandb-project', default='efficientvit-pruning', type=str,
                        help='WandB project name')
    parser.add_argument('--wandb-run-name', default='', type=str,
                        help='WandB run name (비워두면 자동 생성)')

    # Soft Pruning parameters
    parser.add_argument('--pruning', action='store_true',
                        help='Enable soft pruning during training')
    parser.add_argument('--target-compression', type=float, default=0.30,
                        help='Target compression rate: 0.30 / 0.50 / 0.75'
                             ' (= fraction of total params to remove)')
...
def main(args):
    ...
    if args.wandb and utils.is_main_process():
        ...
        wandb.watch(model_without_ddp, log='gradients', log_freq=500)
    if args.resume:
    ...
    pruner = None
    if args.pruning:
        from pruning.efficientvit_pruning import EfficientViTPruner
        pruner = EfficientViTPruner(
            model=model_without_ddp,
            target_compression=args.target_compression,
        )
        print(f"[Pruning] Soft pruning enabled. target_compression={args.target_compression:.0%}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            pruner=pruner,                 # soft pruning (None이면 비활성화)
        )
